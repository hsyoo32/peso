...
from datasets_ori import EmbDataset
...
def parse_args():
    ...
    parser.add_argument('--str', type=str, default='', help='str')
...
ckpt_path = args_setting.root_path + args_setting.dataset + args_setting.str + '/' + args_setting.checkpoint


output_dir = f"./data/{dataset}/"
output_file = f"{dataset}.index.epoch{args_setting.epoch}{args_setting.str}.json"
output_file = os.path.join(output_dir, output_file)
device = torch.device(f"cuda:{args_setting.gpu_id}")
...
#print number of collisions before
tot_item = len(all_indices_str)
tot_indice = len(set(all_indices_str.tolist()))
print("Number of collisions before: ", tot_item-tot_indice)
print("Collision Rate before: ",(tot_item-tot_indice)/tot_item)
...
model.rq.vq_layers[-1].sk_epsilon = 0.003
print('sk_epsilon is ', model.rq.vq_layers[-1].sk_epsilon)
...
while True:
    ...
    # print('tt is ', tt)
    # print('number of collisions is ', len(get_collision_item(all_indices_str)))
    # print("Max number of conflicts: ", max(get_indices_count(all_indices_str).values()))

    # tot_item = len(all_indices_str)
    # tot_indice = len(set(all_indices_str.tolist()))
    # print("Number of collisions ???: ", tot_item-tot_indice)
...
print("All indices number after: ",len(all_indices))
print("Max number of conflicts after: ", max(get_indices_count(all_indices_str).values()))

tot_item = len(all_indices_str)
tot_indice = len(set(all_indices_str.tolist()))
print("Number of collisions after: ", tot_item-tot_indice)
print("Collision Rate after: ",(tot_item-tot_indice)/tot_item)
...
# output_file = output_file.replace('.json', '_nocollision.json')
