...
class Trainer(object):
    ...
    def __init__(self, args, model, train_dataset=None):
        self.args = args
        self.model = model
        self.logger = logging.getLogger()
        self.train_dataset = train_dataset  # Store the training dataset
        ...
        self.ckpt_dir = os.path.join(args.ckpt_dir, args.dataset + "_edim" + str(args.e_dim) + '_beta' + str(args.beta))
    ...
    def _uses_diversity_loss(self):
        return float(getattr(self.args, "beta", 0.0)) > 0.0

    def _refresh_diversity_labels(self):
        if not self._uses_diversity_loss():
            return

        embs = [layer.embedding.weight.cpu().detach().numpy() for layer in self.model.rq.vq_layers]
        for idx, emb in enumerate(embs):
            _, labels = self.constrained_km(emb)
    ...
    def constrained_km(self, data, n_clusters=10):
        ...
        x = data
        # Ensure n_clusters is not too large
        n_clusters = min(n_clusters, len(data) // 4)  # Ensure at least 4 points per cluster on average
        size_min = max(2, min(len(data) // (n_clusters * 2), 10))  # Ensure size_min is at least 2
    ...
    def _train_epoch(self, train_data, epoch_idx):
        ...
        #             )
        self._refresh_diversity_labels()

        for batch_idx, data in enumerate(train_data):
            data, emb_idx = data[0], data[1]
            data = data.to(self.device)
            self.optimizer.zero_grad()
            out, rq_loss, indices, dense_out = self.model(data, self.labels) # Labels = None
    ...
    def _valid_epoch(self, valid_data):
        ...
        for batch_idx, data in enumerate(iter_data):
            ...
            num_sample += len(data)
            data = data.to(self.device)
            indices = self.model.get_indices(data, self.labels) # Labels = None
    ...
    def _save_checkpoint(self, epoch, collision_rate=1, ckpt_file=None):
        ...
        ckpt_path = os.path.join(self.ckpt_dir, 'epoch_%d_collision_%.4f_model_best_collision.pth' % (epoch, collision_rate)) if ckpt_file \
